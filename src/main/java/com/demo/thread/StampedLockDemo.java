...
public class StampedLockDemo {
    ...
    public void write() {
        ...
        try {
            log.info("start write, number: {}, stamp: {}", number, stamp);
            number = number + 1;
        } finally {
            log.info("write end, number: {}, stamp: {}", number, stamp);
    ...
    public void tryOptimisticRead() {
        ...
        log.info("tryOptimisticRead start, number: {}, stamp: {}", number, stamp);
        try {
        ...
        // stampedLock.validate(stamp): true无修改，false有修改
        if (stampedLock.validate(stamp)) {
            log.info("tryOptimisticRead, number: {}, stamp: {}, ", number, stamp);
        } else {
            log.info("有人修改过，有写操作，从乐观读升级为悲观读，并重新获取数据");
            stamp = stampedLock.readLock();
            try {
                log.info("reread number: {}, stamp: {}", number, stamp);
            } finally {
                stampedLock.unlockRead(stamp);
            }
        }
        log.info("tryOptimisticRead end, number: {}, stamp: {}", number, stamp);
    }
    ...
    public void test() {
        ...
        for (int i = 0; i < LOOP_SIZE; i++) {
            new Thread(() -> tryOptimisticRead(), "optimistic_read_thread_" + i).start();
        }
